...
MB.startNotifPoll = function() {
  ...
  MB._notifPollTimer = setInterval(async () => {
    ...
    // ── Whispers ──
    try {
      const r = await fetch('http://localhost:3000/__whisper/poll?name=' + encodeURIComponent(myName));
      if (r.ok) {
        const msgs = await r.json();
        if (msgs && msgs.length) {
          if (MB._notifChannel) MB._notifChannel.postMessage({ type:'whispers', msgs });
          msgs.forEach(m => { if (MB._handleIncomingWhisper) MB._handleIncomingWhisper(m); });
        }
      }
    } catch(e) {}
    // ── Social ──
    try {
      const r = await fetch('http://localhost:3000/__social/poll?name=' + encodeURIComponent(myName));
      if (r.ok) {
        const incoming = await r.json();
        let changed = false;
        const key = 'mythbound_social_v1_' + myName;
        const social = (function(){ try { return JSON.parse(localStorage.getItem(key)||'{"friends":[],"requests":[],"invites":[],"blocked":[]}'); } catch(e){return{friends:[],requests:[],invites:[],blocked:[]};} })();
        (incoming.requests||[]).forEach(fr=>{ if(!(social.requests||[]).includes(fr)){if(!social.requests)social.requests=[];social.requests.push(fr);changed=true;} });
        (incoming.invites||[]).forEach(inv=>{ if(!social.invites)social.invites=[];social.invites.push(inv);changed=true; });
        if (changed) { localStorage.setItem(key,JSON.stringify(social)); if(MB._notifChannel)MB._notifChannel.postMessage({type:'social_update'}); if(MB._handleSocialUpdate)MB._handleSocialUpdate(); }
      }
    } catch(e) {}
  }, 2500);
};
